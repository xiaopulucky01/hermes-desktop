...
Models often glue headings, table headers, and data rows onto one line (e.g. `让我## 标题` or `| ~5MB | | 桌面壳 |`), join whole tables with consecutive pipes (`| a | b | ||------|| | c |`), emit bare numbered digest rows that omit the leading pipe (`1 | news | src || 2 | news2 | src2 |` after a `| 新闻 | 来源 |` header), prefix a row with a stray leading `||`, glue a header directly to a compact separator (`| 目标 | … | ||-------|----------|-----|`), merge header labels (`Skill Agent`), glue multiple data rows via empty-cell boundaries (`| r1 | | r2 |`), emit a one-cell separator (`|------|`) for a multi-column header, omit the GFM separator row, or insert blank lines between rows (which breaks the table and leaves orphan `| 学习系统 | …` text). Real fenced code is never touched — repairs run before [[src/renderer/src/components/AgentMarkdown.tsx]] parses the message. Compact dash separators are rewritten to spaced GFM form (`| --- | --- | --- |`) when column counts already match. [[src/renderer/src/screens/Chat/mediaUtils.ts#normalizeMarkdownTablePunctuation]] first rewrites fullwidth `｜` (common from Qwen/CJK output) and CR/LS/PS line endings to ASCII `|` / `\n`, then [[src/renderer/src/screens/Chat/mediaUtils.ts#repairGluedBarePipeTableRows]] splits bare numbered digests on `||` and folds the leading index into the title cell when the header is only two columns. Inline backticks inside table cells are kept intact during repair so cells like `` 创建 `/.well-known/agent.json` `` are not split mid-row.
...
- **Glued headings** — `###标题` / `文字## 标题` (hashes glued to preceding prose) are broken onto their own line with a space inserted before the title text. The break requires a non-space/non-pipe character before `#`, so table cells like `| # | 新闻 |` are never mistaken for ATX headings.
...
### Unclosed fence keeps call args and block comments intact

Multi-line call argument lists ending in `,` and `/* … */` bodies that contain markdown-looking bullets must stay inside an unclosed fence so Windows junction snippets and following helpers are not split into bare lines plus extra fences.
...
Fenced blocks use Prism with theme-aware one-light / one-dark palettes after language aliases are resolved.

[[src/renderer/src/components/prismLanguage.ts#resolvePrismLanguage]] maps fence aliases (`ts`, `py`, `js`, `yml`, …) and auto-detects language for unlabeled fences from code shape before [[src/renderer/src/components/AgentMarkdown.tsx]] renders tokens.
