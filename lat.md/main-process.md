...
Local chat's preferred `/api/ws` path boots headless `hermes serve`, not `hermes dashboard`, so a missing SPA/`npm` install cannot stall every send.

[[src/main/dashboard.ts#startDashboard]] (local mode) and [[src/main/hermes.ts#TuiGatewayClient]] share one `hermes serve` child: [[src/main/dashboard-launch.ts#buildTuiGatewayCliArgs]] builds `serve --no-open --host 127.0.0.1 --port <n>` on an OS-assigned free port (not a fixed 9120 scan that raced with zombies and hit Windows EADDRINUSE). Concurrent starts collapse via an in-flight promise; readiness is capped at ~45s. The TUI client only attaches a WebSocket to that shared serve — it never spawns a second Python process. After `session.create`/`resume`, [[src/main/hermes.ts#sendMessageViaTuiGateway]] grace-waits briefly for `session.info` before `prompt.submit` — see [[chat-commands#First-token send path]].
...
- **Dashboard** (`hermes dashboard` / headless `hermes serve`, session-token auth) serves the model library, session list (`/api/*`), and the chat **WebSocket** (`/api/ws`) — surfaces the gateway api_server does not. Local chat prefers `/api/ws` via [[main-process#Local TUI gateway]] (`serve`); over SSH the renderer's dashboard transport uses it too, when a dashboard is available.
