...
Streamed reasoning and tool calls collapse into compact transcript rows so heavy turns stay scannable.

When a turn includes `a2a_delegate` / `a2a_call`, [[src/renderer/src/screens/Chat/HistoryRow.tsx#ToolActivityGroup]] shows an always-visible “Used expert” badge and a peer hint so users can tell Hermes actually delegated.
...
Each user/assistant bubble shows an absolute `yyyy-MM-dd HH:mm:ss` label just outside the bubble's bottom-right corner so the transcript is always scrutable for _when_ something was said.

Live bubbles stamp `Date.now()` as soon as they are created — [[src/renderer/src/screens/Chat/hooks/useChatActions.ts#useChatActions|pushUser]] for outgoing user rows, and the first assistant insert in [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts]] / [[src/renderer/src/screens/Chat/hooks/useChatIPC.ts]] for streamed replies — so the label appears without waiting for DB refresh. History still hydrates from state.db: [[src/renderer/src/screens/Chat/sessionHistory.ts#dbItemsToChatMessages]] copies each row's `timestamp`, and [[src/renderer/src/screens/Chat/sessionHistory.ts#reconcileAfterDbRefresh|the end-of-stream reconcile]] can adopt a missing DB time onto a streamed bubble (via `mergeDbMetadataIntoStreamed`) without remounting. state.db stores times in **seconds**, so `coerceToEpochMs` in MessageRow scales any sub-`1e12` value up to milliseconds before use (otherwise it renders as ~Jan 1970). [[src/renderer/src/screens/Chat/MessageRow.tsx#formatBubbleTime]] builds the label with date-fns `format` as `yyyy-MM-dd HH:mm:ss`, and the `<time>` element also carries an ISO `dateTime`. Bubble and timestamp sit in a shrink-wrapped `.chat-bubble-stack` (`width: max-content`); the time is `position: absolute` under the bubble's right edge so a short message (e.g. `"2"`) is not stretched to the timestamp's width, and `.chat-message` `padding-bottom` keeps the label inside paint containment.
